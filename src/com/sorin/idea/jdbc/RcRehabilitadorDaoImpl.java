...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
		...
		try {
			...
			if (dto.isIdProblemaArticularModified()) {
				if (dto.isIdProblemaArticularNull()) {
					...
					stmt.setInt( index++, dto.getIdProblemaArticular() );
				}
			...
			if (dto.isIdProblemaCognitivoModified()) {
				if (dto.isIdProblemaCognitivoNull()) {
					...
					stmt.setInt( index++, dto.getIdProblemaCognitivo() );
				}
			...
			if (dto.isIdProblemaSensorialModified()) {
				if (dto.isIdProblemaSensorialNull()) {
					...
					stmt.setInt( index++, dto.getIdProblemaSensorial() );
				}
			...
			if (dto.isIdNivelEducativoModified()) {
				if (dto.isIdNivelEducativoNull()) {
					...
					stmt.setInt( index++, dto.getIdNivelEducativo() );
				}
			...
			if (rs != null && rs.next()) {
				dto.setIdRcRehabilitador( rs.getInt( 1 ) );
			}
	...
	public void update(RcRehabilitadorPk pk, RcRehabilitador dto) throws RcRehabilitadorDaoException
	{
		...
		try {
			...
			stmt.setInt( index++, pk.getIdRcRehabilitador() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
	...
	public void delete(RcRehabilitadorPk pk) throws RcRehabilitadorDaoException
	{
		...
		try {
			...
			stmt.setInt( 1, pk.getIdRcRehabilitador() );
			int rows = stmt.executeUpdate();
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
		}
	...
	{
		RcRehabilitador ret[] = findByDynamicSelect( SQL_SELECT + " WHERE ID_RC_REHABILITADOR = ?", new Object[] {  new Integer(idRcRehabilitador) } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_PROBLEMA_ARTICULAR = ? ORDER BY ID_PROBLEMA_ARTICULAR", new Object[] {  new Integer(idProblemaArticular) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_PROBLEMA_COGNITIVO = ? ORDER BY ID_PROBLEMA_COGNITIVO", new Object[] {  new Integer(idProblemaCognitivo) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_PROBLEMA_SENSORIAL = ? ORDER BY ID_PROBLEMA_SENSORIAL", new Object[] {  new Integer(idProblemaSensorial) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_NIVEL_EDUCATIVO = ? ORDER BY ID_NIVEL_EDUCATIVO", new Object[] {  new Integer(idNivelEducativo) } );
	}
	...
	protected void populateDto(RcRehabilitador dto, ResultSet rs) throws SQLException
	{
		...
		dto.setIdProblemaArticular( rs.getInt( COLUMN_ID_PROBLEMA_ARTICULAR ) );
		if (rs.wasNull()) {
			dto.setIdProblemaArticularNull( true );
		}
		
		dto.setIdProblemaCognitivo( rs.getInt( COLUMN_ID_PROBLEMA_COGNITIVO ) );
		if (rs.wasNull()) {
			dto.setIdProblemaCognitivoNull( true );
		}
		
		dto.setIdProblemaSensorial( rs.getInt( COLUMN_ID_PROBLEMA_SENSORIAL ) );
		if (rs.wasNull()) {
			dto.setIdProblemaSensorialNull( true );
		}
		
		dto.setIdNivelEducativo( rs.getInt( COLUMN_ID_NIVEL_EDUCATIVO ) );
		if (rs.wasNull()) {
