...
public class ReporteFLE {
	private static Logger logger = Logger.getLogger(ReporteFLE.class);
	PacienteAplDto paciente = new PacienteAplDto();
	ArrTestFle atp = new ArrTestFle();
	...
	public ReporteFLE(String path, ArrTestFle atp, PacienteAplDto pacienteSelect,InfoCentro c, ResourceBundle bundle) {
	...
	public void GeneraArchivoPlanilla() {
		...
		try {
			...
			root.addContent(new Element("test").setText("Flecainida"));
			...
			if(this.atp.getResultado() != null && this.atp.getResultado() > 0){
				String res = maedao.findByPrimaryKey(this.atp.getResultado()).getValor(); 
				root.addContent(new Element("res1").setText(res));
				root.addContent(new Element("res2").setText(null));
				if(res != null && res.equals("POSITIVO")){
					String v1 = "";
					if(this.atp.getV1() == 1) v1 = this.bundle.getString("testfarma_fle_res_resultado_msg_v1")+" "+this.bundle.getString("testfarma_fle_res_resultado_msg_v1_op1");
					if(this.atp.getV1() == 2) v1 = this.bundle.getString("testfarma_fle_res_resultado_msg_v1")+" "+this.bundle.getString("testfarma_fle_res_resultado_msg_v1_op2");
					String v2 = "";
					if(v1.length()>0) v2 += " -";
					if(this.atp.getV2() == 1) v2 += this.bundle.getString("testfarma_fle_res_resultado_msg_v2")+" "+this.bundle.getString("testfarma_fle_res_resultado_msg_v2_op1");
					if(this.atp.getV2() == 2) v2 += this.bundle.getString("testfarma_fle_res_resultado_msg_v2")+" "+this.bundle.getString("testfarma_fle_res_resultado_msg_v2_op2");
					root.addContent(new Element("res3").setText(v1 + v2));
				}else
			...
			e.printStackTrace();
			logger.error("Error generando fichero XML para informe - Reporte Test Flecainida. Motivo: "+e.getMessage(),e);
		}
