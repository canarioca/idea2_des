...
public class ReporteATP {
	private static Logger logger = Logger.getLogger(ReporteATP.class);
	PacienteAplDto paciente = new PacienteAplDto();
	ArrTestAtp atp = new ArrTestAtp();
	...
	public ReporteATP(String path, ArrTestAtp atp, PacienteAplDto pacienteSelect, InfoCentro c, ResourceBundle bundle) {
	...
	public void GeneraArchivoPlanilla() {
		...
		try {
			...
			if(this.atp.getDosis1() != null && this.atp.getDosis1() > 0)
				root.addContent(new Element("res1").setText(maedao.findByPrimaryKey(this.atp.getDosis1()).getValor()));
			else
				root.addContent(new Element("res1").setText(" "));
			if(this.atp.getDosis2() != null && this.atp.getDosis2() > 0)
				root.addContent(new Element("res2").setText(maedao.findByPrimaryKey(this.atp.getDosis2()).getValor()));
			else
				root.addContent(new Element("res2").setText(" "));
			...
			e.printStackTrace();
			logger.error("Error generando fichero XML para informe - Reporte Test ATP. Motivo: "+e.getMessage(),e);
		}
		...
		try {
			...
			e.printStackTrace();
			logger.error("Error generando fichero XML para informe - Test ATP. Motivo: "+e.getMessage(),e);
		}
	}
}
