...
<ui:composition xmlns="http://www.w3.org/1999/xhtml"
	...
	<h:panelGrid id="tablaPacientesBdu" width="100%">
		...
		<t:dataTable styleClass="hor-minimalist-b" var="linea" width="90%"
			binding="#{controlTest.bindingfle }" id="tablafle" rows="13" 
			...
			<t:column >
				...
				<a4j:commandLink value="#{linea.fechaEpisodiostr}" reRender="pantestfle"
					actionListener="#{controlTest.guardaFle}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodiosfle');"
			...
			<t:column>
				...
				<a4j:commandLink value="#{linea.procedimiento}" reRender="pantestfle"
					actionListener="#{controlTest.guardaFle}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodiosfle');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column >
				...
				<a4j:commandLink title="#{ms.episodios_episodio }: #{linea.numicu}"
					value="#{linea.numicu}" reRender="pantestfle"
					actionListener="#{controlTest.guardaFle}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodiosfle');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column>
				...
				<a4j:commandLink value="#{linea.identev}" reRender="pantestfle"
					actionListener="#{controlTest.guardaFle}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodiosfle');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<f:facet name="footer">
				<rich:datascroller for="tablafle" id="scrollepisodiosfle"
					renderIfSinglePage="false" fastStep="4" stepControls="auto"
					boundaryControls="auto" fastControls="auto" />
			</f:facet>
		</t:dataTable>

		<a4j:commandLink title="#{ms.guardar }" reRender="pantestfle"
