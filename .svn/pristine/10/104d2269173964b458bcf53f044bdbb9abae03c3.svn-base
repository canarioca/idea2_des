...
public class ArrTestEpiDaoImpl extends AbstractDAO implements ArrTestEpiDao
{
...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT CODIGO_PACIENTE, CODIGO_TEST, CLINICO, ECG, INTERVALOS, QRS, DOSIS1, DOSIS2, COMPLICA, TRATAM, PICOQT, FECHA, COMPLICAOTR, NPROC  FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( CODIGO_PACIENTE, CODIGO_TEST, CLINICO, ECG, INTERVALOS, QRS, DOSIS1, DOSIS2, COMPLICA, TRATAM, PICOQT, FECHA, COMPLICAOTR, NPROC  ) VALUES ( ?, null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) ";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET CODIGO_PACIENTE = ?, CODIGO_TEST = ?, CLINICO = ?, ECG = ?, INTERVALOS = ?, QRS = ?, DOSIS1 = ?, DOSIS2 = ?, COMPLICA = ?, TRATAM = ?, PICOQT = ?, FECHA = ?, COMPLICAOTR = ?, NPROC = ? WHERE CODIGO_TEST = ?";
	...
	/** 
	 * Index of column PICOQT
	 */
	protected static final int COLUMN_PICOQT = 11;
	...
	public ArrTestEpiPk insert(ArrTestEpi dto) throws ArrTestEpiDaoException
	{
		...
		try {
			...
			stmt.setString( index++, dto.getPicoqt() );
			...
			throw new ArrTestEpiDaoException( "Exception: " + _e.getMessage(), _e );
		}
		finally {
	...
	{
		ArrTestEpi ret[] = findByDynamicSelect( SQL_SELECT + " WHERE CODIGO_TEST = ?", new Object[] { codigoTest } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE DOSIS1 = ? ORDER BY DOSIS1", new Object[] {  new Long(dosis1) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE DOSIS2 = ? ORDER BY DOSIS2", new Object[] {  new Long(dosis2) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE PICOQT = ? ORDER BY PICOQT", new Object[] { picoqt } );
	}
	...
	public ArrayList<ArrayList<Object>> getEstadisticas(Date fechaini,Date fechafin,ArrTestEpi test) throws ArrTestEpiDaoException
	{
		...
		sql.append(" arr_test_epi_ecg ecg");
		...
		sql.append(" arr_test_epi epi");
		sql.append(" left outer join arr_maestras m1 on epi.clinico = m1.codigo");
		sql.append(" left outer join arr_maestras m2 on epi.ecg = m2.codigo");
		sql.append(" left outer join arr_maestras m3 on epi.dosis1 = m3.codigo");
		sql.append(" left outer join arr_maestras m4 on epi.dosis2 = m4.codigo");
		sql.append(" left outer join arr_maestras m5 on epi.complica = m5.codigo");
		sql.append(" left outer join arr_maestras m6 on epi.tratam = m6.codigo");
		sql.append(" where pac.codigo = epi.codigo_paciente");
		sql.append(" and epi.codigo_test = ecg.idtest");
		sql.append(" and epi.fecha between '"+UtilFechas.getFechaString(fechaini)+"' and '"+UtilFechas.getFechaString(fechafin)+"'");
	...
	private String generaWhere(ArrTestEpi test){
		String where = "";
		if(test.getClinico() != null && test.getClinico() > 0)
			where += " and epi.clinico = " + test.getClinico();
		if(test.getEcg() != null && test.getEcg() > 0)
			where += " and epi.ecg = " + test.getEcg();
		if(test.getDosis1() != null && test.getDosis1() > 0)
			where += " and epi.dosis1 = " + test.getDosis1();
		if(test.getComplica() != null && test.getComplica() > 0)
			where += " and epi.complica = " + test.getComplica();
		return where;
	}

	/**
	 * Method 'ArrTestEpiDaoImpl'
	 * 
	 */
	public ArrTestEpiDaoImpl(String usuario)
	{
		this.auditoriaDto = new AuditoriaDto(usuario, getTableName());
	}
	public ArrTestEpiDaoImpl(final java.sql.Connection userConn,String usuario)
	{
	 ...
	 * @param userConn
	 */
	public ArrTestEpiDaoImpl(final java.sql.Connection userConn)
	{
		this.userConn = userConn;
	}
	 ...
	 */
	protected ArrTestEpi fetchSingleResult(ResultSet rs) throws SQLException
	{
		if (rs.next()) {
			ArrTestEpi dto = new ArrTestEpi();
			populateDto( dto, rs);
			return dto;
		} else {
			return null;
	 ...
	 */
	protected ArrTestEpi[] fetchMultiResults(ResultSet rs) throws SQLException
	{
		...
		ArrTestEpi ret[] = new ArrTestEpi[ resultList.size() ];
	 ...
	 */
	protected void populateDto(ArrTestEpi dto, ResultSet rs) throws SQLException
	{
		...
		dto.setPicoqt( rs.getString( COLUMN_PICOQT ) );
	 ...
	 */
	protected void reset(ArrTestEpi dto)
	{
	}
	 ...
	 */
	public ArrayList<ArrayList<Object>> findByDynamicSelect(String sql) throws ArrTestEpiDaoException
	{
		// declare variables
		final boolean isConnSupplied = (userConn != null);
		Connection conn = null;
	...
	public ArrayList<ArrayList<Object>> getEstadisticasEPI(Date fechaini, Date fechafin) throws ArrTestEpiDaoException{
		...
		sql.append(" and proc.tipoprocedimiento = 1064 ");
	...
	@Override
	public ArrTestEpi[] findWhereComplicaotrEquals(String complicaotr)
			throws ArrTestEpiDaoException {
