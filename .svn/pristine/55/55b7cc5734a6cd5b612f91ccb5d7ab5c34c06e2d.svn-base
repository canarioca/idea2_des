	...
	<h:panelGrid width="100%">
		...
		<h:panelGrid columns="3" width="100%"
			...
			<h:panelGrid id="tablaRitmos"
				style="border-top: solid 1px;border-color: black;">
				<h:selectOneMenu value="#{controlTest.atp.ecgdto.idritmo }">
			...
			<h:panelGrid id="tablaCond"
				...
				<h:panelGrid columns="2">
					...
					<h:selectOneRadio layout="pageDirection"
						value="#{controlTest.atp.ecgdto.idconduccion }" onclick="updecg();">
						<f:selectItem itemLabel="#{ms.testfarma_ecg_si }" itemValue="2" />
						<f:selectItem itemLabel="#{ms.testfarma_ecg_no }" itemValue="1" />
					</h:selectOneRadio>
					<rich:spacer width="35px"
						rendered="#{controlTest.atp.ecgdto.idconduccion eq 1 }" />
					<h:panelGrid columns="2"
						rendered="#{controlTest.atp.ecgdto.idconduccion eq 1 }">
						<rich:spacer width="35px" />
						<h:selectOneMenu value="#{controlTest.atp.ecgdto.idconduccionno }">
			...
			<h:panelGrid id="tablaQrs"
				...
				<h:panelGrid columns="2">
					...
					<h:selectOneRadio layout="pageDirection" onclick="updecgqrs();"
						value="#{controlTest.atp.ecgdto.idqrs}">
						<f:selectItem itemLabel="#{ms.testfarma_ecg_si }" itemValue="2" />
						<f:selectItem itemLabel="#{ms.testfarma_ecg_no }" itemValue="1" />
					</h:selectOneRadio>
					<rich:spacer width="35px"
						rendered="#{controlTest.atp.ecgdto.idqrs eq 1 }" />
					<h:panelGrid columns="2"
						rendered="#{controlTest.atp.ecgdto.idqrs eq 1 }">
						<rich:spacer width="35px" />
						<h:selectOneMenu value="#{controlTest.atp.ecgdto.idqrsno }">
		...
		<h:panelGrid columns="2" width="80%"
			...
			<h:panelGrid width="90%" columns="3" id="intervalos">
				...
				<h:inputText size="8" maxlength="5"
					value="#{controlTest.atp.ecgdto.rr }" />
				<h:outputLabel value="#{ms.testfarma_ecg_ms }." />
				<h:outputLabel value="#{ms.testfarma_ecg_pr }" />
				<h:inputText size="8" maxlength="5"
					value="#{controlTest.atp.ecgdto.pr }" />
				<h:outputLabel value="#{ms.testfarma_ecg_ms }." />
				<h:outputLabel value="#{ms.testfarma_ecg_qrs }" />
				<h:inputText size="8" maxlength="5"
					value="#{controlTest.atp.ecgdto.qrs }" />
				<h:outputLabel value="#{ms.testfarma_ecg_ms }." />
				<h:outputLabel value="#{ms.testfarma_ecg_qt }" />
				<h:inputText size="8" maxlength="5"
					value="#{controlTest.atp.ecgdto.qt }"
					onblur="updqtc();" />
				<h:outputLabel value="#{ms.testfarma_ecg_ms }." />
				<h:outputLabel value="#{ms.testfarma_ecg_qtc }" />
				<h:inputText size="8" readonly="true"
					value="#{controlTest.atp.ecgdto.qtc }" />
				<h:outputLabel value="#{ms.testfarma_ecg_ms }." />
			</h:panelGrid>
			<h:panelGrid width="90%"
				style="border-top: solid 1px;border-color: black;">
				<h:inputTextarea cols="80" rows="5" id="comentariosAtpEcg"
					value="#{controlTest.atp.ecgdto.comentarios }" >
					<f:validateLength maximum="2000" />
				</h:inputTextarea>	
				<h:message for="comentariosAtpEcg" style="color:red" />
			</h:panelGrid>
		</h:panelGrid>
	</h:panelGrid>
</ui:composition>