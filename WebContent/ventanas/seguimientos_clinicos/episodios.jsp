...
<ui:composition xmlns="http://www.w3.org/1999/xhtml"
	...
	<h:panelGrid id="tablaPacientesBdu" width="100%">
		<h:outputText rendered="#{empty controlSeguimientosClinicos.episodios}"
			value="#{ms.episodios_sin_resultados }"
			style="font-size: 16px;font-weight: bold;" />
		<t:dataTable styleClass="hor-minimalist-b" var="linea" width="90%"
			binding="#{controlSeguimientosClinicos.bindingepi}" id="tablaepi" rows="13"
			preserveDataModel="true" value="#{controlSeguimientosClinicos.episodios}">			
			<t:column id="fechaepi">
				<f:facet name="header">
					<h:outputText value="#{ms.episodios_fechaproc }" />
				</f:facet>
				<h:outputText value="#{linea.fechaEpisodiostr}" rendered="false"/>
				<a4j:commandLink value="#{linea.fechaEpisodiostr}" reRender="panicosegclin,panComplicacionesPacienteSegClin"
					actionListener="#{controlSeguimientosClinicos.guarda}"
			...
			<t:column>
				...
				<a4j:commandLink value="#{linea.procedimiento}" reRender="panicosegclin,panComplicacionesPacienteSegClin"
					actionListener="#{controlSeguimientosClinicos.guarda}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column id="episodio">
				...
				<a4j:commandLink title="#{ms.episodios_episodio }: #{linea.numicu}"
					value="#{linea.numicu}" reRender="panicosegclin"
					actionListener="#{controlSeguimientosClinicos.guarda}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column>
				...
				<a4j:commandLink value="#{linea.identev}" reRender="panicosegclin,panComplicacionesPacienteSegClin"
					actionListener="#{controlSeguimientosClinicos.guarda}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
		...
		<a4j:commandLink title="#{ms.guardar }" reRender="panSeguimientoRegEv,panicosegclin,panComplicacionesPacienteSegClin"
