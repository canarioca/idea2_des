...
<ui:composition xmlns="http://www.w3.org/1999/xhtml"
	...
	<h:panelGrid id="tablaPacientesBdu" width="100%">
		<h:outputText rendered="#{empty controlEef.episodios}"
			value="#{ms.episodios_sin_resultados }"
			style="font-size: 16px;font-weight: bold;" />
		<t:dataTable styleClass="hor-minimalist-b" var="linea" width="90%"
			binding="#{controlEef.bindingepi}" id="tablaepi" rows="13"
			preserveDataModel="true" value="#{controlEef.episodios}">
			...
			<t:column id="fechaepi">
				...
				<a4j:commandLink value="#{linea.fechaEpisodiostr}" reRender="paniconos,panComplicacionesEefDiag,panComplicacionesPacienteEefAbl"
			...
			<t:column>
				...
				<a4j:commandLink value="#{linea.procedimiento}" reRender="paniconos,panComplicacionesEefDiag,panComplicacionesPacienteEefAbl"
					actionListener="#{controlEef.guardaEstudio}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column id="episodio">
				...
				<a4j:commandLink title="#{ms.episodios_episodio }: #{linea.numicu}"
					value="#{linea.numicu}" reRender="paniconos,panComplicacionesEefDiag,panComplicacionesPacienteEefAbl"
					actionListener="#{controlEef.guardaEstudio}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
			<t:column>
				...
				<a4j:commandLink value="#{linea.identev}" reRender="paniconos,panComplicacionesEefDiag,panComplicacionesPacienteEefAbl"
					actionListener="#{controlEef.guardaEstudio}"
					onclick="javascript:Richfaces.showModalPanel('buscando');javascript:Richfaces.hideModalPanel('episodios');"
					oncomplete="javascript:Richfaces.hideModalPanel('buscando');" />
			</t:column>
		...
		<a4j:commandLink title="#{ms.guardar }" reRender="paniconos,panComplicacionesEefDiag,panComplicacionesPacienteEefAbl"
